// server.js - LOTATO PRO v6.0 — CORS corrigé pour APK Capacitor
...
`);
  ...
  await pool.query(`ALTER TABLE tickets ADD COLUMN IF NOT EXISTS win_details JSONB`).catch(() => {});
  await pool.query(`ALTER TABLE lottery_settings ADD COLUMN IF NOT EXISTS address TEXT`).catch(() => {});
  await pool.query(`ALTER TABLE lottery_settings ADD COLUMN IF NOT EXISTS phone_numbers TEXT`).catch(() => {});
  await pool.query(`ALTER TABLE lottery_settings ADD COLUMN IF NOT EXISTS advanced_settings JSONB`).catch(() => {});
  await pool.query(`ALTER TABLE draws ADD COLUMN IF NOT EXISTS is_special BOOLEAN DEFAULT false`).catch(() => {});
  await pool.query(`ALTER TABLE draws ADD COLUMN IF NOT EXISTS day_of_week INTEGER`).catch(() => {});
  await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS subscription_status VARCHAR(20)`).catch(() => {});
  await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS last_payment_date TIMESTAMP`).catch(() => {});
  await pool.query(`ALTER TABLE users ADD COLUMN IF NOT EXISTS subscription_expiry_date TIMESTAMP`).catch(() => {});
...
function scheduleMidnightReactivation() {
    ...
    setTimeout(async () => {
        try {
            const result = await pool.query(`UPDATE draws SET active = true WHERE active = false`);
    ...
    console.log(`⏰ Prochaine réactivation dans ${Math.round(delay / 1000 / 60)} minutes`);
}
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO players (name, phone, password, zone, owner_id, balance, created_at, updated_at)
       VALUES ($1, $2, $3, $4, $5, 0, NOW(), NOW()) RETURNING id, name, phone, balance`,
...
app.post('/api/auth/player/login', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      'SELECT id, name, phone, password, balance, owner_id FROM players WHERE phone = $1', [phone]
    );
    ...
    res.json({ success: true, token, playerId: player.id, name: player.name, balance: parseFloat(player.balance), ownerId: player.owner_id });
  } catch (err) {
...
app.get('/api/lottery-settings', authenticate, async (req, res) => {
  ...
  try {
    ...
    res.json({ name: row.name, slogan: row.slogan, logoUrl: row.logo_url, multipliers: row.multipliers, limits: row.limits, address: row.address || '', phone_numbers: row.phone_numbers || '' });
  ...
  try {
    ...
    const draw = await pool.query(
      `SELECT l.draw_id, d.name as draw_name, l.number, l.limit_amount
       ...
       WHERE l.owner_id = $1 ORDER BY draw_id, number`, [ownerId]
    );
...
app.post('/api/tickets/save', authenticate, async (req, res) => {
  ...
  try {
    ...
    if (drawCheck.rows.length === 0) return res.status(404).json({ error: 'Tirage introuvable' });
    ...
    if (req.user.role !== 'superadmin') {
        if (draw.is_special) {
            ...
            if (access.rows.length === 0) return res.status(403).json({ error: 'Ce tirage spécial ne vous est pas accessible' });
        }
        if (draw.day_of_week !== null) {
            const currentDay = new Date().getDay();
            if (currentDay !== draw.day_of_week) return res.status(403).json({ error: "Ce tirage n'est pas disponible aujourd'hui" });
        }
    }

    if (!draw.active) return res.status(403).json({ error: 'Tirage bloqué par administrateur' });

    const [hours, minutes] = draw.time.split(':');
    const now = moment().tz('America/Port-au-Prince');
    const drawTimeFull = moment().tz('America/Port-au-Prince').set({ hour: parseInt(hours), minute: parseInt(minutes), second: 0 });
    if (now.isAfter(drawTimeFull)) return res.status(403).json({ error: `Tirage déjà terminé.` });
    const blockFrom = drawTimeFull.clone().subtract(3, 'minutes');
    if (now.isSameOrAfter(blockFrom)) return res.status(403).json({ error: `Tirage fermé depuis ${blockFrom.format('HH:mm')} (3 min avant).` });
    ...
    for (const bet of bets) {
      ...
      if (game === 'borlette' || game === 'BO' || (game && game.startsWith('n'))) normalized = rawNumber.padStart(2, '0');
      else if (game === 'lotto3' || game === 'auto_lotto3') normalized = rawNumber.padStart(3, '0');
      ...
      if ((game === 'lotto3' || game === 'auto_lotto3') && normalized.length === 3 && blockedLotto3Set.has(normalized)) {
        return res.status(403).json({ error: `Numéro Lotto3 ${normalized} est bloqué` });
      }
      if (drawLimitsMap.has(normalized) && !bet.free) {
        ...
        if (current + amount > limit) exceeded.push({ type: 'tirage', number: normalized, limit, already: current, requested: amount, remaining: limit - current });
      }
      if (globalLimitsMap.has(normalized) && !bet.free) {
        ...
        const amount = parseFloat(bet.amount) || 0;
        if (current + amount > limit) exceeded.push({ type: 'global', number: normalized, limit, already: current, requested: amount, remaining: limit - current });
      }
    }
    if (exceeded.length > 0) {
      const message = exceeded.map(e => `Numéro ${e.number} (${e.type === 'global' ? 'limite globale' : 'limite tirage'}) : limite ${e.limit} G, déjà ${e.already} G, reste ${e.remaining} G.`).join('\n');
    ...
    for (const [category, total] of Object.entries(totalsByGame)) {
      ...
      if (limit > 0 && total > limit) return res.status(403).json({ error: `Limite de mise pour ${category} dépassée (max ${limit} Gdes)` });
    }
    ...
    for (let i = 0; i < requiredFree; i++) {
      ...
      newFreeBets.push({ game: 'auto_marriage', number: `${n1}&${n2}`, cleanNumber: n1 + n2, amount: 0, free: true, freeType: 'special_marriage', freeWin: 2500 });
    }
...
app.get('/api/tickets', authenticate, async (req, res) => {
  ...
  let idx = 2;
  if (user.role === 'agent') { query += ` AND agent_id = $${idx++}`; params.push(user.id); }
    ...
    if (agentId) {
      ...
      query += ` AND agent_id = $${idx++}`; params.push(agentId);
    } else { query += ` AND agent_id IN (SELECT id FROM users WHERE supervisor_id = $${idx++} AND role = 'agent')`; params.push(user.id); }
  } else if (user.role === 'owner') {
    const { agentId } = req.query;
...
app.get('/api/reports', authenticate, async (req, res) => {
  ...
  try {
    ...
    const row = result.rows[0];
    res.json({ totalTickets: parseInt(row.total_tickets), totalBets: parseFloat(row.total_bets), totalWins: parseFloat(row.total_wins), totalLoss: parseFloat(row.total_bets) - parseFloat(row.total_wins), balance: parseFloat(row.balance) });
  } catch (err) { res.status(500).json({ error: 'Erreur serveur' }); }
});
...
app.get('/api/agent/reports', authenticate, async (req, res) => {
  ...
  let paramIndex = 3;
  if (drawId && drawId !== 'all') { conditions.push(`t.draw_id = $${paramIndex++}`); params.push(drawId); }
  let dateCondition = '';
  if (period === 'today') dateCondition = 'DATE(t.date) = CURRENT_DATE';
  else if (period === 'yesterday') dateCondition = "DATE(t.date) = CURRENT_DATE - INTERVAL '1 day'";
  ...
  else if (period === 'custom' && fromDate && toDate) { dateCondition = `DATE(t.date) BETWEEN $${paramIndex++} AND $${paramIndex++}`; params.push(fromDate, toDate); }
  if (dateCondition) conditions.push(dateCondition);
  const whereClause = conditions.join(' AND ');
  try {
    const summaryResult = await pool.query(`SELECT COUNT(*) as total_tickets, COALESCE(SUM(t.total_amount), 0) as total_bets, COALESCE(SUM(t.win_amount), 0) as total_wins, COALESCE(SUM(t.win_amount) - SUM(t.total_amount), 0) as net_result FROM tickets t WHERE ${whereClause}`, params);
    const summary = summaryResult.rows[0];
    let detail = [];
    if (!drawId || drawId === 'all') {
      const detailResult = await pool.query(`SELECT d.name as draw_name, d.id as draw_id, COUNT(t.id) as tickets, COALESCE(SUM(t.total_amount), 0) as bets, COALESCE(SUM(t.win_amount), 0) as wins, COALESCE(SUM(t.win_amount) - SUM(t.total_amount), 0) as result FROM tickets t JOIN draws d ON t.draw_id = d.id WHERE ${whereClause} GROUP BY d.id, d.name ORDER BY result DESC`, params);
  ...
  try {
    const result = await pool.query(
      `SELECT wr.*, d.name as draw_name, wr.date as published_at
       ...
       ORDER BY wr.draw_id, wr.date DESC`, [ownerId]
    );
    const rows = result.rows.map(row => ({ ...row, numbers: typeof row.numbers === 'string' ? JSON.parse(row.numbers) : row.numbers, published_at: row.published_at, name: row.draw_name }));
  ...
  try {
    const result = await pool.query(
      `SELECT COUNT(t.id) as total_tickets, COALESCE(SUM(t.total_amount),0) as total_bets,
              COALESCE(SUM(t.win_amount),0) as total_wins, COALESCE(SUM(t.win_amount)-SUM(t.total_amount),0) as balance
       FROM tickets t JOIN users u ON t.agent_id = u.id
       WHERE t.owner_id = $1 AND u.supervisor_id = $2 AND DATE(t.date) = CURRENT_DATE`, [ownerId, supervisorId]
    );
  ...
  try {
    const result = await pool.query(
      `SELECT u.id, u.name, u.username, u.blocked, u.zone,
              ...
              COUNT(t.id) as total_tickets, COALESCE(SUM(t.win_amount)-SUM(t.total_amount),0) as balance,
       ...
       WHERE u.owner_id = $1 AND u.supervisor_id = $2 AND u.role = 'agent' GROUP BY u.id`, [ownerId, supervisorId]
    );
    res.json(result.rows);
  } catch (err) { res.status(500).json({ error: 'Erreur serveur' }); }
});
  ...
  try {
    const result = await pool.query(
      `SELECT t.* FROM tickets t JOIN users u ON t.agent_id = u.id
       ...
       ORDER BY t.date DESC LIMIT 20`, [ownerId, supervisorId, agentId]
    );
    res.json(result.rows);
  } catch (err) { res.status(500).json({ error: 'Erreur serveur' }); }
});
...
app.get('/api/supervisor/tickets', authenticate, requireRole('supervisor'), async (req, res) => {
  ...
  const countRes = await pool.query(query.replace('SELECT t.*', 'SELECT COUNT(*)'), params);
  ...
  try {
    const check = await pool.query(
      `SELECT t.id FROM tickets t JOIN users u ON t.agent_id = u.id
       WHERE t.id = $1 AND t.owner_id = $2 AND u.supervisor_id = $3`, [ticketId, ownerId, supervisorId]
    );
  ...
  try {
    const result = await pool.query(
      `SELECT u.id, u.name, u.username, u.blocked, u.zone, u.cin, u.commission_percentage, s.name as supervisor_name
       ...
       WHERE u.owner_id = $1 AND u.role = $2`, [ownerId, 'agent']
    );
  ...
  try {
    const result = await pool.query(
      `SELECT b.draw_id, d.name as draw_name, b.number
       FROM draw_blocked_numbers b JOIN draws d ON b.draw_id = d.id WHERE b.owner_id = $1`, [ownerId]
    );
    res.json(result.rows);
  } catch (err) { res.status(500).json({ error: 'Erreur' }); }
});
...
app.get('/api/owner/number-limits', authenticate, requireRole('owner'), async (req, res) => {
  ...
  try {
    ...
    const draw = await pool.query(
      `SELECT l.draw_id, d.name as draw_name, l.number, l.limit_amount
       FROM draw_number_limits l LEFT JOIN draws d ON l.draw_id = d.id
       WHERE l.owner_id = $1 ORDER BY l.draw_id, l.number`, [ownerId]
    );
    res.json([...global.rows, ...draw.rows]);
  } catch (err) { res.status(500).json({ error: 'Erreur' }); }
});
...
app.post('/api/owner/number-limit', authenticate, requireRole('owner'), async (req, res) => {
  ...
  const normalized = number.padStart(2, '0');
  if (!/^\d{2}$/.test(normalized)) return res.status(400).json({ error: 'Numéro invalide' });
  ...
  try {
    ...
    const agentsGainLoss = await pool.query(
      `SELECT u.id, u.name, COALESCE(SUM(t.total_amount),0) as total_bets, COALESCE(SUM(t.win_amount),0) as total_wins, COALESCE(SUM(t.win_amount)-SUM(t.total_amount),0) as net_result
       FROM users u LEFT JOIN tickets t ON u.id = t.agent_id AND DATE(t.date) = CURRENT_DATE
       WHERE u.owner_id = $1 AND u.role = $2 GROUP BY u.id`, [ownerId, 'agent']
    );
    res.json({
...
app.get('/api/owner/reports', authenticate, requireRole('owner'), async (req, res) => {
  ...
  if (period === 'today') baseQuery += ` AND DATE(date AT TIME ZONE 'America/Port-au-Prince') = (NOW() AT TIME ZONE 'America/Port-au-Prince')::DATE`;
  else if (period === 'yesterday') baseQuery += ` AND DATE(date AT TIME ZONE 'America/Port-au-Prince') = (NOW() AT TIME ZONE 'America/Port-au-Prince')::DATE - INTERVAL '1 day'`;
  else if (period === 'week') baseQuery += ` AND date AT TIME ZONE 'America/Port-au-Prince' >= DATE_TRUNC('week', NOW() AT TIME ZONE 'America/Port-au-Prince')`;
  else if (period === 'month') baseQuery += ` AND date AT TIME ZONE 'America/Port-au-Prince' >= DATE_TRUNC('month', NOW() AT TIME ZONE 'America/Port-au-Prince')`;
  else if (period === 'custom' && fromDate && toDate) { baseQuery += ` AND DATE(date AT TIME ZONE 'America/Port-au-Prince') BETWEEN $${idx++} AND $${idx++}`; params.push(fromDate, toDate); }
  ...
  res.json({ summary: { total_tickets: parseInt(summary.tickets), total_bets: parseFloat(summary.bets), total_wins: parseFloat(summary.wins), net_result: parseFloat(summary.result) } });
});
  ...
  try {
    await pool.query(
      `INSERT INTO lottery_settings (owner_id, name, slogan, logo_url, multipliers, limits, address, phone_numbers)
       ...
       ON CONFLICT (owner_id) DO UPDATE SET name = EXCLUDED.name, slogan = EXCLUDED.slogan, logo_url = EXCLUDED.logo_url, multipliers = EXCLUDED.multipliers, limits = EXCLUDED.limits, address = EXCLUDED.address, phone_numbers = EXCLUDED.phone_numbers`,
...
app.get('/api/owner/advanced-settings', authenticate, requireRole('owner'), async (req, res) => {
  const ownerId = req.user.id;
  try {
    const result = await pool.query(`SELECT advanced_settings FROM lottery_settings WHERE owner_id = $1`, [ownerId]);
    const defaults = { freeMarriage: { tiers: [{ min: 0, max: 50, count: 1 }, { min: 51, max: 150, count: 2 }, { min: 151, max: null, count: 3 }], winAmount: 1000 }, print: { fontSize: 32 }, footer: { line1: "tickets valable jusqu'à 90 jours", line2: "Ref : +509 ", line3: "LOTATO S.A." } };
    if (result.rows.length === 0 || !result.rows[0].advanced_settings) return res.json(defaults);
...
app.post('/api/owner/advanced-settings', authenticate, requireRole('owner'), async (req, res) => {
  ...
  try {
    await pool.query(
      `INSERT INTO lottery_settings (owner_id, advanced_settings) VALUES ($1, $2)
        ...
        for (const bet of bets) {
          ...
          if (game === 'borlette' || game === 'BO' || (game && game.startsWith('n'))) {
            if (clean.length === 2) {
              ...
              if (gain > 0) { win_details.push({ game, number: clean, gain }); totalWin += gain; }
            }
          } else if (game === 'lotto3' || game === 'auto_lotto3') {
            if (clean.length === 3 && clean === lotto3) { gain = amount * multipliers.lotto3; win_details.push({ game, number: clean, gain }); totalWin += gain; }
          } else if (game === 'mariage' || game === 'auto_marriage') {
            if (clean.length === 4) {
              const first = clean.slice(0,2), second = clean.slice(2,4);
              const pairs = [lot1, lot2, lot3_num];
              let win = false;
              for (let i=0; i<3; i++) { for (let j=0; j<3; j++) { if (i !== j && first === pairs[i] && second === pairs[j]) { win = true; break; } } if (win) break; }
              if (win) {
                ...
                if (bet.free && bet.freeType === 'special_marriage') {
                  ...
                  gain = freeWinAmount;
                } else { gain = amount * multipliers.mariage; }
                win_details.push({ game, number: `${first}&${second}`, gain }); totalWin += gain;
              }
            }
            ...
            if (clean.length === 4 && bet.option) {
              ...
              if (clean === expected) { gain = amount * multipliers.lotto4; win_details.push({ game, number: clean, gain }); totalWin += gain; }
            }
            ...
            if (clean.length === 5 && bet.option) {
              ...
              if (clean === expected) { gain = amount * multipliers.lotto5; win_details.push({ game, number: clean, gain }); totalWin += gain; }
            }
          }
        }
      }
      await pool.query('UPDATE tickets SET win_amount = $1, checked = true, win_details = $2 WHERE id = $3', [totalWin, JSON.stringify(win_details), ticket.id]);
    }
...
app.post('/api/superadmin/owners', authenticate, requireSuperAdmin, async (req, res) => {
  ...
  try {
    const hashed = await bcrypt.hash(password, 10);
    const result = await pool.query(
      `INSERT INTO users (name, username, password, role, phone, quota, created_at, blocked) VALUES ($1, $2, $3, 'owner', $4, $5, NOW(), false) RETURNING id, name, username`,
...
app.get('/api/superadmin/agents', authenticate, requireSuperAdmin, async (req, res) => {
  try {
    const result = await pool.query(`SELECT u.id, u.name, u.username as email, u.phone, u.owner_id, o.name as owner_name FROM users u LEFT JOIN users o ON u.owner_id = o.id WHERE u.role = 'agent' ORDER BY u.name`);
    res.json(result.rows);
  } catch (err) { res.status(500).json({ error: 'Erreur serveur' }); }
});
...
app.put('/api/superadmin/agents/:id', authenticate, requireSuperAdmin, async (req, res) => {
  ...
  try {
    ...
    if (password && password.trim() !== '') {
      const hashed = await bcrypt.hash(password, 10);
      query += ', password = $5'; params.push(hashed);
      query += ` WHERE id = $${params.length + 1} AND role = 'agent'`; params.push(id);
    } else { query += ` WHERE id = $${params.length + 1} AND role = 'agent'`; params.push(id); }
    const result = await pool.query(query, params);
...
app.get('/api/superadmin/supervisors', authenticate, requireSuperAdmin, async (req, res) => {
  try {
    const result = await pool.query(`SELECT u.id, u.name, u.username as email, u.phone, u.owner_id, o.name as owner_name FROM users u LEFT JOIN users o ON u.owner_id = o.id WHERE u.role = 'supervisor' ORDER BY u.name`);
    res.json(result.rows);
  } catch (err) { res.status(500).json({ error: 'Erreur serveur' }); }
});
...
app.put('/api/superadmin/supervisors/:id', authenticate, requireSuperAdmin, async (req, res) => {
  ...
  try {
    ...
    if (password && password.trim() !== '') {
      const hashed = await bcrypt.hash(password, 10);
      query += ', password = $5'; params.push(hashed);
      query += ` WHERE id = $${params.length + 1} AND role = 'supervisor'`; params.push(id);
    } else { query += ` WHERE id = $${params.length + 1} AND role = 'supervisor'`; params.push(id); }
    const result = await pool.query(query, params);
...
app.post('/api/superadmin/messages/bulk', authenticate, requireSuperAdmin, async (req, res) => {
  ...
  if (!ownerIds || !Array.isArray(ownerIds) || ownerIds.length === 0 || !message) return res.status(400).json({ error: 'Liste et message requis' });
  try {
    const values = ownerIds.map((_, i) => `($${i*2+1}, $${i*2+2}, NOW(), NOW() + INTERVAL '10 minutes')`).join(',');
  ...
  try {
    const result = await pool.query(`
      SELECT u.id, u.name, u.username as email,
             COUNT(DISTINCT ag.id) as agent_count, COUNT(t.id) as ticket_count,
             COALESCE(SUM(t.total_amount), 0) as total_bets, COALESCE(SUM(t.win_amount), 0) as total_wins,
...
app.post('/api/superadmin/owners/:id/pay', authenticate, requireSuperAdmin, async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE users SET subscription_status = 'active', last_payment_date = $1, subscription_expiry_date = $2 WHERE id = $3 AND role = 'owner' RETURNING id`,
      [now, expiry, id]
    );
...
app.get('/api/superadmin/draw-access/:drawId', authenticate, requireSuperAdmin, async (req, res) => {
    ...
    try {
        const result = await pool.query(`
            SELECT u.id as owner_id, u.name as owner_name, COALESCE(oda.enabled, false) as enabled
            FROM users u LEFT JOIN owner_draw_access oda ON oda.owner_id = u.id AND oda.draw_id = $1
            WHERE u.role = 'owner' ORDER BY u.name
        `, [drawId]);
        res.json(result.rows);
    } catch (err) { res.status(500).json({ error: 'Erreur serveur' }); }
});
...
app.put('/api/superadmin/draw-access/:drawId/:ownerId', authenticate, requireSuperAdmin, async (req, res) => {
    ...
    try {
        await pool.query(`
            INSERT INTO owner_draw_access (owner_id, draw_id, enabled, updated_at) VALUES ($1, $2, $3, NOW())
            ON CONFLICT (owner_id, draw_id) DO UPDATE SET enabled = $3, updated_at = NOW()
        `, [ownerId, drawId, enabled]);
        res.json({ success: true });
    } catch (err) { res.status(500).json({ error: 'Erreur mise à jour accès' }); }
});
...
app.post('/api/superadmin/publish-results', authenticate, requireSuperAdmin, async (req, res) => {
  ...
  try {
    ...
    if (ownerCheck.rows.length === 0) return res.status(404).json({ error: 'Propriétaire introuvable' });
    await pool.query(`INSERT INTO winning_results (owner_id, draw_id, numbers, lotto3, date) VALUES ($1, $2, $3, $4, NOW())`, [ownerId, drawId, JSON.stringify(numbers), lotto3]);
    const settingsRes = await pool.query('SELECT multipliers FROM lottery_settings WHERE owner_id = $1', [ownerId]);
    let multipliers = { lot1: 60, lot2: 20, lot3: 10, lotto3: 500, lotto4: 5000, lotto5: 25000, mariage: 500 };
    if (settingsRes.rows.length > 0 && settingsRes.rows[0].multipliers) {
    ...
    for (const ticket of ticketsRes.rows) {
      ...
      if (Array.isArray(bets)) {
        for (const bet of bets) {
          ...
          let gain = 0;
          if (game === 'borlette' || game === 'BO' || (game && game.startsWith('n'))) {
            if (clean.length === 2) { if (clean === lot1) gain = amount * multipliers.lot1; else if (clean === lot2) gain = amount * multipliers.lot2; else if (clean === lot3_num) gain = amount * multipliers.lot3; }
          } else if (game === 'lotto3' || game === 'auto_lotto3') { if (clean.length === 3 && clean === lotto3) gain = amount * multipliers.lotto3; }
          else if (game === 'mariage' || game === 'auto_marriage') {
            if (clean.length === 4) {
              const first = clean.slice(0,2), second = clean.slice(2,4);
              const pairs = [lot1, lot2, lot3_num];
            ...
            if (clean.length === 4 && bet.option) {
              let expected = '';
              if (bet.option == 1) expected = lot1 + lot2; else if (bet.option == 2) expected = lot2 + lot3_num; else if (bet.option == 3) expected = lot1 + lot3_num;
              if (clean === expected) gain = amount * multipliers.lotto4;
            }
          } else if (game === 'lotto5' || game === 'auto_lotto5') {
            if (clean.length === 5 && bet.option) {
              let expected = '';
              if (bet.option == 1) expected = lotto3 + lot2; else if (bet.option == 2) expected = lotto3 + lot3_num;
    ...
    await pool.query(`INSERT INTO activity_log (user_id, user_role, action, details, ip_address) VALUES ($1, $2, $3, $4, $5)`, [req.user.id, 'superadmin', 'publish_results', `Owner ${ownerId}, Draw ${drawId}`, req.ip]);
    res.json({ success: true });
  } catch (err) { console.error(err); res.status(500).json({ error: 'Erreur serveur' }); }
});
...
app.post('/api/superadmin/publish-results-bulk', authenticate, requireSuperAdmin, async (req, res) => {
  ...
  if (!ownerIds || !Array.isArray(ownerIds) || ownerIds.length === 0) return res.status(400).json({ error: 'Liste ownerIds requise' });
  if (!drawId || !numbers || numbers.length !== 3 || !lotto3) return res.status(400).json({ error: 'drawId, numbers, lotto3 requis' });
  async function publishForOneOwner(ownerId) {
    try {
      const ownerCheck = await pool.query('SELECT id FROM users WHERE id = $1 AND role = $2', [ownerId, 'owner']);
      if (ownerCheck.rows.length === 0) return { success: false, ownerId, error: 'Propriétaire introuvable' };
      await pool.query(`INSERT INTO winning_results (owner_id, draw_id, numbers, lotto3, date) VALUES ($1, $2, $3, $4, NOW())`, [ownerId, drawId, JSON.stringify(numbers), lotto3]);
      const settingsRes = await pool.query('SELECT multipliers FROM lottery_settings WHERE owner_id = $1', [ownerId]);
      let multipliers = { lot1: 60, lot2: 20, lot3: 10, lotto3: 500, lotto4: 5000, lotto5: 25000, mariage: 500 };
      if (settingsRes.rows.length > 0 && settingsRes.rows[0].multipliers) { const raw = settingsRes.rows[0].multipliers; multipliers = typeof raw === 'string' ? JSON.parse(raw) : raw; }
      const [lot1, lot2, lot3_num] = numbers;
      const ticketsRes = await pool.query('SELECT id, bets FROM tickets WHERE owner_id = $1 AND draw_id = $2 AND checked = false', [ownerId, drawId]);
      for (const ticket of ticketsRes.rows) {
        let totalWin = 0;
        ...
        if (Array.isArray(bets)) {
          for (const bet of bets) {
            ...
            const amount = parseFloat(bet.amount) || 0;
            let gain = 0;
            if (game === 'borlette' || game === 'BO' || (game && game.startsWith('n'))) { if (clean.length === 2) { if (clean === lot1) gain = amount * multipliers.lot1; else if (clean === lot2) gain = amount * multipliers.lot2; else if (clean === lot3_num) gain = amount * multipliers.lot3; } }
            else if (game === 'lotto3' || game === 'auto_lotto3') { if (clean.length === 3 && clean === lotto3) gain = amount * multipliers.lotto3; }
            else if (game === 'mariage' || game === 'auto_marriage') { if (clean.length === 4) { const first = clean.slice(0,2), second = clean.slice(2,4); const pairs = [lot1, lot2, lot3_num]; let win = false; for (let i=0; i<3; i++) { for (let j=0; j<3; j++) { if (i !== j && first === pairs[i] && second === pairs[j]) { win = true; break; } } if (win) break; } if (win) gain = amount * multipliers.mariage; } }
            else if (game === 'lotto4' || game === 'auto_lotto4') { if (clean.length === 4 && bet.option) { let expected = ''; if (bet.option == 1) expected = lot1 + lot2; else if (bet.option == 2) expected = lot2 + lot3_num; else if (bet.option == 3) expected = lot1 + lot3_num; if (clean === expected) gain = amount * multipliers.lotto4; } }
            else if (game === 'lotto5' || game === 'auto_lotto5') { if (clean.length === 5 && bet.option) { let expected = ''; if (bet.option == 1) expected = lotto3 + lot2; else if (bet.option == 2) expected = lotto3 + lot3_num; if (clean === expected) gain = amount * multipliers.lotto5; } }
            totalWin += gain;
          }
        }
        await pool.query('UPDATE tickets SET win_amount = $1, checked = true WHERE id = $2', [totalWin, ticket.id]);
      }
      await pool.query(`INSERT INTO activity_log (user_id, user_role, action, details, ip_address) VALUES ($1, $2, $3, $4, $5)`, [req.user.id, 'superadmin', 'publish_results_bulk', `Owner ${ownerId}, Draw ${drawId}`, req.ip]);
      return { success: true, ownerId };
    } catch (err) { return { success: false, ownerId, error: err.message }; }
  }
  const results = await Promise.all(ownerIds.map(id => publishForOneOwner(id)));
  const successCount = results.filter(r => r.success).length;
  res.json({ success: true, summary: { total: results.length, success: successCount, failed: results.length - successCount }, results });
});
...
app.post('/api/owner/players', authenticate, requireRole('owner'), async (req, res) => {
  ...
  try {
    ...
    if (err.code === '23505') return res.status(400).json({ error: 'Ce numéro existe déjà' });
